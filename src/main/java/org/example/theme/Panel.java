...
public class Panel extends PanelBase<IPanelItem> {
    ...
    public void render(RenderContext context, double mouseX, double mouseY) {
        ...
        double rowHeight = 15;
        renderer.drawOutlinedRectangle(x, y, getWidth(), rowHeight, ExamplePlugin.theme.outlineWidth.getValue(), ExamplePlugin.theme.categoryColor.getValueRGB(), ExamplePlugin.theme.outlineColor.getValueRGB());

        double categoryTextHeight = y + rowHeight / 2 - getFontRenderer().getFontHeight() / 2;
        getFontRenderer().drawString(category, x + 3F, categoryTextHeight, ExamplePlugin.theme.getColorSetting().getValue().getRGB());

        double categoryTitleOffset = getFontRenderer().getStringWidth(category + " ");
        getFontRenderer().drawString("(" + String.valueOf(moduleItems.size()) + ")", x + 3F + categoryTitleOffset, categoryTextHeight, ExamplePlugin.theme.fontColor.getValue().getRGB());

        if(open) {
            if (height > 0) {
                renderer.drawOutlinedRectangle(x, y+rowHeight, getWidth(), height + 1.5F, ExamplePlugin.theme.outlineWidth.getValue(),
                        ExamplePlugin.theme.backColor.getValueRGB(),
                        ExamplePlugin.theme.outlineColor.getValueRGB());
            }
            double y0 = y + rowHeight + 1.5F;
            if (height > 0) {
